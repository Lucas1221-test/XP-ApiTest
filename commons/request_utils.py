...
from commons.yaml_utils import write_yaml
from hotloads.debug_talk import DebugTalk
from commons.base_url import get_base_url
from commons.logs_config import logger
...
class RequestUtils:
    ...
    def standard_yaml_case(self, caseinfo):
        ...
        if "post-operation" in caseinfo:
            res = self.precise_filtering(res, caseinfo)
            print(f"处理后的{res}")
    ...
    def precise_filtering(self, res, caseinfo):
        path = caseinfo["post-operation"]["path"]
        print(path)
        key = caseinfo["post-operation"]["key"]
        print(key)
        value = caseinfo["post-operation"]["value"]
        print(value)
        # 使用 jsonpath 提取匹配结果
        js_value = jsonpath.jsonpath(res.json(), path)
        print("js_value的值：%s" % js_value)
        new_res = {}
        # 如果没有匹配到结果
        if not js_value:
        ...
        for i in js_value:
            ...
            if isinstance(i, dict) and i.get(key) == value:
                new_res = i
        new_res = json.dumps(new_res, ensure_ascii=False, indent=4)
        print(type(new_res))
        if not res:
        ...
        # 设置新响应对象的属性
        new_response.status_code = res.status_code  # 复制状态码
        new_response._content = json.dumps(new_res, ensure_ascii=False, indent=4).encode('utf-8')  # 设置响应内容
        new_response.headers = res.headers  # 复制响应头
